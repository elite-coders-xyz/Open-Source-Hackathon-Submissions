# Open Source Hackathon 2026 Project Submission

## Participant Details

**Full Name:**  
Harish Ganth

**GitHub Username:**  
Harishganth-0704

**Team Name:**  
Solo Developer

**College/University:**  
Hindusthan Institute of Technology

---

## Project Details

**Project Title:**  
CareerCraft: AI-Powered Career Path Recommendation & Smart Recruitment Platform

**Project Description:**  
CareerCraft is a full-stack AI-powered Super App that bridges the gap between job seekers and recruiters. Features include AI chatbot powered by Google Gemini for career assessments, interactive roadmaps for Data Science, AI Engineering, Full Stack and Cybersecurity with real-time progress tracking, gamified dashboard with points/streaks/badges, AI Resume Builder with ATS PDF export, 1-click Portfolio Generator, and a B2B HR portal for smart candidate matchmaking.

**Tech Stack Used:**  
React 19, Vite 7, Node.js, Express.js, MongoDB (Mongoose), Google Gemini AI API, JWT Authentication, bcryptjs, Nodemailer, Helmet, Express-Rate-Limit, Vanilla CSS, Vercel (Frontend), Render (Backend)

**GitHub Repository Link:**  
https://github.com/Harishganth-0704/AI-Based-Smart-Career-Guidance-Recruitment-Platform

**Live Demo Link:**  
https://ai-based-smart-career-guidance-recruitment-platform.vercel.app

**Presentation / Demo Video Link:**  
https://github.com/Harishganth-0704/AI-Based-Smart-Career-Guidance-Recruitment-Platform/blob/main/project_demo_video.webp

---

## Open Source Readiness

- [x] My project is public on GitHub
- [x] My repository has a proper README.md
- [x] I have added setup/installation instructions
- [x] I have added screenshots/demo where possible
- [x] I have added a license file
- [x] My project is original and built/updated during the hackathon period

---

## Memori Labs Sponsor Task

- [x] I have starred the Memori Labs GitHub repository  
  https://github.com/MemoriLabs/Memori

- [x] I have followed Memori Labs on LinkedIn  
  https://www.linkedin.com/company/memorilabs/

- [x] I have followed Memori Labs on X  
  https://x.com/memorilab

- [x] I have checked Memori Labs social links  
  https://linktr.ee/memorilabs

---

## ID Card Verification

- [x] I have generated my ID card from https://oshack.xyz
- [ ] If my ID was not verified, I completed the mandatory verification/giveaway form and tried again

---