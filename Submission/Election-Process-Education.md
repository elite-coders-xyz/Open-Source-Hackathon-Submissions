# Open Source Hackathon 2026 Project Submission

## Participant Details

**Full Name:**  
Jagriti

**GitHub Username:**  
Jagriti-student

**Team Name:**  
SoloCoder

**College/University:**  
Guru Jambheshwar University of Science & Technology

---

## Project Details

**Project Title:**  
AI-Powered Election Education Assistant 🗳️🤖

**Project Description:**  
-An interactive AI-powered civic education platform designed to help first-time voters understand the democratic election process in a simple and engaging way.  

-The system uses a structured election timeline (Registration → Nomination → Campaign → Voting → Results), an interactive voting simulator, quizzes, misinformation awareness tools, and a Google Gemini AI assistant to provide personalized civic guidance.

-This project solves the problem of low voter awareness and election confusion by making civic education interactive, visual, and AI-driven.


**Tech Stack Used:**  
- React (Vite)  
- TypeScript  
- Tailwind CSS  
- Google Gemini 1.5 Flash API  
- Vitest  

**GitHub Repository Link:**  
https://github.com/Jagriti-student/Election-Process-Education

**Live Demo Link:**  
https://election-system-495006.web.app

**Presentation / Demo Video Link:**  
<!-- Optional but recommended -->

---

## Open Source Readiness

- [x] My project is public on GitHub
- [x] My repository has a proper README.md
- [x] I have added setup/installation instructions
- [x] I have added screenshots/demo where possible
- [x] I have added a license file
- [x] My project is original and built/updated during the hackathon period

---

## Memori Labs Sponsor Task

Please complete these before submitting:

- [x] I have starred the Memori Labs GitHub repository  
  https://github.com/MemoriLabs/Memori

- [x] I have followed Memori Labs on LinkedIn  
  https://www.linkedin.com/company/memorilabs/

- [x] I have followed Memori Labs on X  
  https://x.com/memorilab

- [x] I have checked Memori Labs social links  
  https://linktr.ee/memorilabs

---

## ID Card Verification

- [x] I have generated my ID card from https://oshack.xyz
- [x] If my ID was not verified, I completed the mandatory verification/giveaway form and tried again

---
